...
//                 <img src={whatwedo} alt="What We Do" className='w-11/12 md:w-2/5 mt-20 md:mt-10' />
//                 <Link to={'/portfolio'}>
...
//             >
//                 <div className="w-full h-full p-1 md:p-5 lg:p-10">
...
//                             >
//                                 <motion.li
//                                     className='backdrop-blur-sm flex justify-between text-5xl lg:text-7xl md:px-5 py-10 transition-all duration-500 bg-[#ffffff67] rounded-full my-5 hover:tracking-wider'
//                                     variants={listItemVariants}
//                                     transition={{ delay: 0.2 * index, duration: 0.5, ease: 'easeIn' }}
//                                 >
//                                     <p className='px-10'>{item}</p>
//                                     <motion.img
//                                         className='z-50 rounded-lg md:rounded-2xl lg:rounded-3xl hidden group-hover:block absolute right-1/2 bottom-0 w-24 h-16 sm:w-28 sm:h-20 md:w-44 md:h-36  lg:w-64 lg:h-56 float-right object-cover'
...
import emotes from '../assets/emotes.webp';
import animation from '../assets/animations.webp';
...
const OurWork = () => {
    ...
    const handleMouseLeave = () => {
        setHovered(null);
        // setMousePosition({ x: 0, y: 0 }); // Optional: Reset mouse position on leave
    };
    ...
    return (
        ...
        >
            <div id='work' className='flex flex-col md:flex-row justify-between items-center'>
                <img src={whatwedo} alt="What We Do" className='w-11/12 md:w-2/5 mt-20 md:mt-0' />
                <Link to={'/portfolio'}>
                    <button className='hidden md:block text-lg font-semibold px-4 py-3 bg-[#facb0d] mt-10 border border-black drop-shadow-[4px_4px_0px_#1f1f1f] hover:drop-shadow-[1px_1px_0px_#1f1f1f] transition-all duration-200 md:mr-10' style={{ WebkitTextStroke: '0px #1f1f1f', textShadow: 'none' }}>
                        Explore Full Portfolio
                    </button>
                            ...
                            <Link key={index} to={'/portfolio'}>
                                ...
                                >
                                    <motion.li
                                        className='backdrop-blur-sm flex justify-between text-5xl lg:text-7xl md:px-5 py-10 transition-all duration-500 bg-[#ffffff67] rounded-full my-5 hover:tracking-wider'
                                        variants={listItemVariants}
                                        transition={{ delay: 0.1 * index, duration: 0.2, ease: 'easeIn' }}
                                    >
                                        <p className='px-10'>{item}</p>
                                        <motion.img
                                            className='z-50 rounded-lg md:rounded-2xl lg:rounded-3xl hidden group-hover:block absolute right-1/2 bottom-0 w-24 h-16 sm:w-28 sm:h-20 md:w-44 md:h-36  lg:w-64 lg:h-56 float-right object-cover'
